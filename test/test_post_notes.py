...
class TestPostNotes:
    def test_create_note(self, note_creator, delete_note_teardown):
        response = note_creator.creating_note(content="обоев", title="рулон")
        json_response = response.json()
        assert response.status_code == 201
        assert json_response["message"] == "Заметка создана!"

    def test_creating_note_without_token(self, note_creator, delete_note_teardown):
        response = note_creator.creating_note_without_token(content="Банан", title="оранжевый")
        json_response = response.json()
